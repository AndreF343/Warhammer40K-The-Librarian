...
"""
Dependency-free slugify version to avoid accidental import of the wrong `slugify` package (which can call unicode()).
- Removes third-party slugify; uses local simple_slugify.
- Keeps BeautifulSoup/lxml/requests/pandas only.
- Same behavior as v3 otherwise.
"""
...
def simple_slugify(text: str, maxlen: int = 100) -> str:
...
def env_banner():
    ...
    banner = {
        ...
        "versions": {
            ...
            "lxml": ver("lxml"),
            "pandas": ver("pandas"),
            "slugify_module_found": ver("slugify")
        }
    }
...
def write_markdown(page_id: int, title: str, url: str, categories, sections, body_md: str):
    ...
    slug = simple_slugify(title or f"page-{page_id}", 100)
...
def env_banner():
    # defined above; Python requires function before call, but we redefine for clarity in file
    pass

if __name__ == "__main__":
    # Rebind env_banner (Python quirk from duplicate def)
    def env_banner():
        def ver(pkg):
            try:
                m = __import__(pkg)
